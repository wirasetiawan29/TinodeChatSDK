...
class LoginViewController: UIViewController {
    ...
    @IBAction func initialization(_ sender: Any) {
        BBChat().initialization()
    }
    
    @IBAction func startChatClicked(_ sender: Any) {
        BBChat().startChat(view: self, topic: "usr7yG--GVH87o")
    }
    
    
    @IBAction func startCallClicked(_ sender: Any) {
        BBChat().startCall(topic: "usr7yG--GVH87o")
    }
    
    @IBAction func saveDeviceToken(_ sender: Any) {
        BBChat().saveDeviceToken(token: "token12345678910", onSuccess: { data in
    ...
    @IBAction func loginClicked(_ sender: Any) {
        ...
        BBChat().doLogin(userName: userName, password: password) { result in
        
            switch result {
            
            case .success(let data):
                DispatchQueue.main.async { UiUtils.showToast(message: "success \(data.description)")}
                break

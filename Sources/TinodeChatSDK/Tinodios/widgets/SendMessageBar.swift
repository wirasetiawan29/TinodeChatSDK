...
protocol SendMessageBarDelegate: AnyObject {
    ...
    func sendMessageBar(imageFrom: SourceImageType)
}
...
class SendMessageBar: UIView {
    ...
    private enum Constants {
        ...
        static let kSendButtonImageWave = UIImage(named: "img_microphone", in: .main, with: UIImage.SymbolConfiguration(pointSize: Constants.kSendButtonPointsNormal))!
        static let kSendButtonImageWavePressed = UIImage(named: "img_microphone", in: .main, with: UIImage.SymbolConfiguration(pointSize: Constants.kSendButtonPointsPressed))!
        static let kSendButtonImageArrow = UIImage(named: "img_send", in: .main, with: UIImage.SymbolConfiguration(pointSize: Constants.kSendButtonPointsNormal))!
    ...
    @IBOutlet weak var attachCameraButton: UIButton!
    ...
    @IBAction func getFromGallery(_ sender: UIButton) {
        inputField.resignFirstResponder()
        self.delegate?.sendMessageBar(imageFrom: .file)
    }
    
    @IBAction func getFromCamera(_ sender: UIButton) {
        inputField.resignFirstResponder()
        self.delegate?.sendMessageBar(imageFrom: .camera)
    }
    ...
    private func configure() {
        ...
        inputField.layer.borderColor = UIColor(hex: "#DEDEDE").cgColor
    ...
    func showAudioBar(_ state: AudioBarState) {
        ...
        if state == .hidden {
            ...
            attachCameraButton.isHidden = false
            ...
            attachCameraButton.isHidden = true

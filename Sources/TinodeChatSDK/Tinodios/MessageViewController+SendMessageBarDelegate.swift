...
extension MessageViewController: SendMessageBarDelegate {
    ...
    func sendMessageBar(imageFrom: SourceImageType) {
        switch imageFrom {
        case .camera:
            imagePicker?.presentTakeCamera(from: self.view)
        default:
...
//            performSegue(withIdentifier: "ShowImagePreview", sender: content)
            ...
            /// Navigate to Preview Image
            
            let imagePreviewController = ImagePreviewController()
            imagePreviewController.previewContent = content
            self.navigationController?.pushViewController(imagePreviewController, animated: true)
